...
var domContainer=dom(this._rippleContainer||this),target=dom(optTriggeringEvent).rootTarget;if(domContainer.deepContains(/** @type {Node} */target)){this._ripple.uiDownAction(optTriggeringEvent)}}}},/**
   ...
   */_createRipple:function _createRipple(){var element=/** @type {!PaperRippleElement} */document.createElement("paper-ripple");return element},_noinkChanged:function _noinkChanged(noink){if(this.hasRipple()){this._ripple.noink=noink}}};_exports.PaperRippleBehavior=PaperRippleBehavior;var paperRippleBehavior={PaperRippleBehavior:PaperRippleBehavior};_exports.$paperRippleBehavior=paperRippleBehavior;var common=require("./common.js"),System=common.System,VbrMode=common.VbrMode,Float=common.Float,ShortBlock=common.ShortBlock,Util=common.Util,Arrays=common.Arrays,new_array_n=common.new_array_n,new_byte=common.new_byte,new_double=common.new_double,new_float=common.new_float,new_float_n=common.new_float_n,new_int=common.new_int,new_int_n=common.new_int_n,assert=common.assert;Lame=require("./Lame.js");Presets=require("./Presets.js");GainAnalysis=require("./GainAnalysis.js");QuantizePVT=require("./QuantizePVT.js");Quantize=require("./Quantize.js");Takehiro=require("./Takehiro.js");Reservoir=require("./Reservoir.js");MPEGMode=require("./MPEGMode.js");BitStream=require("./BitStream.js");var Encoder=require("./Encoder.js"),Version=require("./Version.js"),VBRTag=require("./VBRTag.js");function GetAudio(){var parse,mpg;this.setModules=function(parse2,mpg2){parse=parse2;mpg=mpg2}}function Parse(){var ver,id3,pre;this.setModules=function(ver2,id32,pre2){ver=ver2;id3=id32;pre=pre2}}function MPGLib(){}function ID3Tag(){var bits,ver;this.setModules=function(_bits,_ver){bits=_bits;ver=_ver}}function Mp3Encoder(channels,samplerate,kbps){if(3!=arguments.length){console.error("WARN: Mp3Encoder(channels, samplerate, kbps) not specified");channels=1;samplerate=44100;kbps=128}var lame=new Lame,gaud=new GetAudio,ga=new GainAnalysis,bs=new BitStream,p=new Presets,qupvt=new QuantizePVT,qu=new Quantize,vbr=new VBRTag,ver=new Version,id3=new ID3Tag,rv=new Reservoir,tak=new Takehiro,parse=new Parse,mpg=new MPGLib;lame.setModules(ga,bs,p,qupvt,qu,vbr,ver,id3,mpg);bs.setModules(ga,mpg,ver,vbr);id3.setModules(bs,ver);p.setModules(lame);qu.setModules(bs,rv,qupvt,tak);qupvt.setModules(tak,rv,lame.enc.psy);rv.setModules(bs);tak.setModules(qupvt);vbr.setModules(lame,bs,ver);gaud.setModules(parse,mpg);parse.setModules(ver,id3,p);var gfp=lame.lame_init();gfp.num_channels=channels;gfp.in_samplerate=samplerate;gfp.brate=kbps;gfp.mode=MPEGMode.STEREO;gfp.quality=3;gfp.bWriteVbrTag=!1;gfp.disable_reservoir=!0;gfp.write_id3tag_automatic=!1;var retcode=lame.lame_init_params(gfp);assert(0==retcode);var maxSamples=1152,mp3buf_size=0|1.25*maxSamples+7200,mp3buf=new_byte(mp3buf_size);this.encodeBuffer=function(left,right){if(1==channels){right=left}assert(left.length==right.length);if(left.length>maxSamples){maxSamples=left.length;mp3buf_size=0|1.25*maxSamples+7200;mp3buf=new_byte(mp3buf_size)}var _sz=lame.lame_encode_buffer(gfp,left,right,left.length,mp3buf,0,mp3buf_size);return new Int8Array(mp3buf.subarray(0,_sz))};this.flush=function(){var _sz=lame.lame_encode_flush(gfp,mp3buf,0,mp3buf_size);return new Int8Array(mp3buf.subarray(0,_sz))}}function WavHeader(){this.dataOffset=0;this.dataLen=0;this.channels=0;this.sampleRate=0}function fourccToInt(fourcc){return fourcc.charCodeAt(0)<<24|fourcc.charCodeAt(1)<<16|fourcc.charCodeAt(2)<<8|fourcc.charCodeAt(3)}WavHeader.RIFF=fourccToInt("RIFF");WavHeader.WAVE=fourccToInt("WAVE");WavHeader.fmt_=fourccToInt("fmt ");WavHeader.data=fourccToInt("data");WavHeader.readHeader=function(dataView){var w=new WavHeader,header=dataView.getUint32(0,!1);if(WavHeader.RIFF!=header){return}var fileLen=dataView.getUint32(4,!0);if(WavHeader.WAVE!=dataView.getUint32(8,!1)){return}if(WavHeader.fmt_!=dataView.getUint32(12,!1)){return}var fmtLen=dataView.getUint32(16,!0),pos=16+4;switch(fmtLen){case 16:case 18:w.channels=dataView.getUint16(pos+2,!0);w.sampleRate=dataView.getUint32(pos+4,!0);break;default:throw"extended fmt chunk not implemented";}pos+=fmtLen;var data=WavHeader.data,len=0;while(data!=header){header=dataView.getUint32(pos,!1);len=dataView.getUint32(pos+4,!0);if(data==header){break}pos+=len+8}w.dataLen=len;w.dataOffset=pos+8;return w};module.exports.Mp3Encoder=Mp3Encoder;module.exports.WavHeader=WavHeader;// Copyright (c) 2013 Pieroxy <pieroxy@pieroxy.net>
...
var LZString$1=function(){// private property
var f=String.fromCharCode,keyStrBase64="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",keyStrUriSafe="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+-$",baseReverseDic={};function getBaseValue(alphabet,character){if(!baseReverseDic[alphabet]){baseReverseDic[alphabet]={};for(var i=0;i<alphabet.length;i++){baseReverseDic[alphabet][alphabet.charAt(i)]=i}}return baseReverseDic[alphabet][character]}var LZString={compressToBase64:function compressToBase64(input){if(null==input)return"";var res=LZString._compress(input,6,function(a){return keyStrBase64.charAt(a)});switch(res.length%4){// To produce valid Base64
default:// When could this happen ?
case 0:return res;case 1:return res+"===";case 2:return res+"==";case 3:return res+"=";}},decompressFromBase64:function decompressFromBase64(input){if(null==input)return"";if(""==input)return null;return LZString._decompress(input.length,32,function(index){return getBaseValue(keyStrBase64,input.charAt(index))})},compressToUTF16:function compressToUTF16(input){if(null==input)return"";return LZString._compress(input,15,function(a){return f(a+32)})+" "},decompressFromUTF16:function decompressFromUTF16(compressed){if(null==compressed)return"";if(""==compressed)return null;return LZString._decompress(compressed.length,16384,function(index){return compressed.charCodeAt(index)-32})},//compress into uint8array (UCS-2 big endian format)
compressToUint8Array:function compressToUint8Array(uncompressed){// 2 bytes per character
for(var compressed=LZString.compress(uncompressed),buf=new Uint8Array(2*compressed.length),i=0,TotalLen=compressed.length,current_value;i<TotalLen;i++){current_value=compressed.charCodeAt(i);buf[2*i]=current_value>>>8;buf[2*i+1]=current_value%256}return buf},//decompress from uint8array (UCS-2 big endian format)
decompressFromUint8Array:function decompressFromUint8Array(compressed){if(null===compressed||compressed===void 0){return LZString.decompress(compressed)}else{// 2 bytes per character
for(var buf=Array(compressed.length/2),i=0,TotalLen=buf.length;i<TotalLen;i++){buf[i]=256*compressed[2*i]+compressed[2*i+1]}var result=[];buf.forEach(function(c){result.push(f(c))});return LZString.decompress(result.join(""))}},//compress into a string that is already URI encoded
compressToEncodedURIComponent:function compressToEncodedURIComponent(input){if(null==input)return"";return LZString._compress(input,6,function(a){return keyStrUriSafe.charAt(a)})},//decompress from an output of compressToEncodedURIComponent
decompressFromEncodedURIComponent:function decompressFromEncodedURIComponent(input){if(null==input)return"";if(""==input)return null;input=input.replace(/ /g,"+");return LZString._decompress(input.length,32,function(index){return getBaseValue(keyStrUriSafe,input.charAt(index))})},compress:function compress(uncompressed){return LZString._compress(uncompressed,16,function(a){return f(a)})},_compress:function _compress(uncompressed,bitsPerChar,getCharFromInt){if(null==uncompressed)return"";var i,value,context_dictionary={},context_dictionaryToCreate={},context_c="",context_wc="",context_w="",context_enlargeIn=2,// Compensate for the first entry which should not count
context_dictSize=3,context_numBits=2,context_data=[],context_data_val=0,context_data_position=0,ii;for(ii=0;ii<uncompressed.length;ii+=1){context_c=uncompressed.charAt(ii);if(!Object.prototype.hasOwnProperty.call(context_dictionary,context_c)){context_dictionary[context_c]=context_dictSize++;context_dictionaryToCreate[context_c]=!0}context_wc=context_w+context_c;if(Object.prototype.hasOwnProperty.call(context_dictionary,context_wc)){context_w=context_wc}else{if(Object.prototype.hasOwnProperty.call(context_dictionaryToCreate,context_w)){if(256>context_w.charCodeAt(0)){for(i=0;i<context_numBits;i++){context_data_val=context_data_val<<1;if(context_data_position==bitsPerChar-1){context_data_position=0;context_data.push(getCharFromInt(context_data_val));context_data_val=0}else{context_data_position++}}value=context_w.charCodeAt(0);for(i=0;8>i;i++){context_data_val=context_data_val<<1|1&value;if(context_data_position==bitsPerChar-1){context_data_position=0;context_data.push(getCharFromInt(context_data_val));context_data_val=0}else{context_data_position++}value=value>>1}}else{value=1;for(i=0;i<context_numBits;i++){context_data_val=context_data_val<<1|value;if(context_data_position==bitsPerChar-1){context_data_position=0;context_data.push(getCharFromInt(context_data_val));context_data_val=0}else{context_data_position++}value=0}value=context_w.charCodeAt(0);for(i=0;16>i;i++){context_data_val=context_data_val<<1|1&value;if(context_data_position==bitsPerChar-1){context_data_position=0;context_data.push(getCharFromInt(context_data_val));context_data_val=0}else{context_data_position++}value=value>>1}}context_enlargeIn--;if(0==context_enlargeIn){context_enlargeIn=Math.pow(2,context_numBits);context_numBits++}delete context_dictionaryToCreate[context_w]}else{value=context_dictionary[context_w];for(i=0;i<context_numBits;i++){context_data_val=context_data_val<<1|1&value;if(context_data_position==bitsPerChar-1){context_data_position=0;context_data.push(getCharFromInt(context_data_val));context_data_val=0}else{context_data_position++}value=value>>1}}context_enlargeIn--;if(0==context_enlargeIn){context_enlargeIn=Math.pow(2,context_numBits);context_numBits++}// Add wc to the dictionary.
context_dictionary[context_wc]=context_dictSize++;context_w=context_c+""}}// Output the code for w.
if(""!==context_w){if(Object.prototype.hasOwnProperty.call(context_dictionaryToCreate,context_w)){if(256>context_w.charCodeAt(0)){for(i=0;i<context_numBits;i++){context_data_val=context_data_val<<1;if(context_data_position==bitsPerChar-1){context_data_position=0;context_data.push(getCharFromInt(context_data_val));context_data_val=0}else{context_data_position++}}value=context_w.charCodeAt(0);for(i=0;8>i;i++){context_data_val=context_data_val<<1|1&value;if(context_data_position==bitsPerChar-1){context_data_position=0;context_data.push(getCharFromInt(context_data_val));context_data_val=0}else{context_data_position++}value=value>>1}}else{value=1;for(i=0;i<context_numBits;i++){context_data_val=context_data_val<<1|value;if(context_data_position==bitsPerChar-1){context_data_position=0;context_data.push(getCharFromInt(context_data_val));context_data_val=0}else{context_data_position++}value=0}value=context_w.charCodeAt(0);for(i=0;16>i;i++){context_data_val=context_data_val<<1|1&value;if(context_data_position==bitsPerChar-1){context_data_position=0;context_data.push(getCharFromInt(context_data_val));context_data_val=0}else{context_data_position++}value=value>>1}}context_enlargeIn--;if(0==context_enlargeIn){context_enlargeIn=Math.pow(2,context_numBits);context_numBits++}delete context_dictionaryToCreate[context_w]}else{value=context_dictionary[context_w];for(i=0;i<context_numBits;i++){context_data_val=context_data_val<<1|1&value;if(context_data_position==bitsPerChar-1){context_data_position=0;context_data.push(getCharFromInt(context_data_val));context_data_val=0}else{context_data_position++}value=value>>1}}context_enlargeIn--;if(0==context_enlargeIn){context_enlargeIn=Math.pow(2,context_numBits);context_numBits++}}// Mark the end of the stream
value=2;for(i=0;i<context_numBits;i++){context_data_val=context_data_val<<1|1&value;if(context_data_position==bitsPerChar-1){context_data_position=0;context_data.push(getCharFromInt(context_data_val));context_data_val=0}else{context_data_position++}value=value>>1}// Flush the last char
while(!0){context_data_val=context_data_val<<1;if(context_data_position==bitsPerChar-1){context_data.push(getCharFromInt(context_data_val));break}else context_data_position++}return context_data.join("")},decompress:function decompress(compressed){if(null==compressed)return"";if(""==compressed)return null;return LZString._decompress(compressed.length,32768,function(index){return compressed.charCodeAt(index)})},_decompress:function _decompress(length,resetValue,getNextValue){var dictionary=[],next,enlargeIn=4,dictSize=4,numBits=3,entry="",result=[],i,w,bits,resb,maxpower,power,c,data={val:getNextValue(0),position:resetValue,index:1};for(i=0;3>i;i+=1){dictionary[i]=i}bits=0;maxpower=Math.pow(2,2);power=1;while(power!=maxpower){resb=data.val&data.position;data.position>>=1;if(0==data.position){data.position=resetValue;data.val=getNextValue(data.index++)}bits|=(0<resb?1:0)*power;power<<=1}switch(next=bits){case 0:bits=0;maxpower=Math.pow(2,8);power=1;while(power!=maxpower){resb=data.val&data.position;data.position>>=1;if(0==data.position){data.position=resetValue;data.val=getNextValue(data.index++)}bits|=(0<resb?1:0)*power;power<<=1}c=f(bits);break;case 1:bits=0;maxpower=Math.pow(2,16);power=1;while(power!=maxpower){resb=data.val&data.position;data.position>>=1;if(0==data.position){data.position=resetValue;data.val=getNextValue(data.index++)}bits|=(0<resb?1:0)*power;power<<=1}c=f(bits);break;case 2:return"";}dictionary[3]=c;w=c;result.push(c);while(!0){if(data.index>length){return""}bits=0;maxpower=Math.pow(2,numBits);power=1;while(power!=maxpower){resb=data.val&data.position;data.position>>=1;if(0==data.position){data.position=resetValue;data.val=getNextValue(data.index++)}bits|=(0<resb?1:0)*power;power<<=1}switch(c=bits){case 0:bits=0;maxpower=Math.pow(2,8);power=1;while(power!=maxpower){resb=data.val&data.position;data.position>>=1;if(0==data.position){data.position=resetValue;data.val=getNextValue(data.index++)}bits|=(0<resb?1:0)*power;power<<=1}dictionary[dictSize++]=f(bits);c=dictSize-1;enlargeIn--;break;case 1:bits=0;maxpower=Math.pow(2,16);power=1;while(power!=maxpower){resb=data.val&data.position;data.position>>=1;if(0==data.position){data.position=resetValue;data.val=getNextValue(data.index++)}bits|=(0<resb?1:0)*power;power<<=1}dictionary[dictSize++]=f(bits);c=dictSize-1;enlargeIn--;break;case 2:return result.join("");}if(0==enlargeIn){enlargeIn=Math.pow(2,numBits);numBits++}if(dictionary[c]){entry=dictionary[c]}else{if(c===dictSize){entry=w+w.charAt(0)}else{return null}}result.push(entry);// Add w+entry[0] to the dictionary.
dictionary[dictSize++]=w+entry.charAt(0);enlargeIn--;w=entry;if(0==enlargeIn){enlargeIn=Math.pow(2,numBits);numBits++}}}};return LZString}();if("function"===typeof define&&define.amd){define(function(){return LZString$1})}else if("undefined"!==typeof module&&null!=module){module.exports=LZString$1}// var LZString = require('lz-string');
// import './lame.js';
// import './lz-string.js';
var $_documentContainer=document.createElement("template");$_documentContainer.setAttribute("style","display: none;");$_documentContainer.innerHTML="<dom-module id=\"voice-button\">\n\n  <!-- \n  Note: deprecation warning for styling master document doesn't apply within <template> tags\n  Read more: https://www.polymer-project.org/blog/2017-10-18-upcoming-changes\n   -->\n\n  <template>\n\n    <style>\n\n      /* shadow DOM styles */      \n      \n      :host {\n        /* red active color */\n        --active-color: #fe4042;\n        /* background color for button */\n        --button-color: #FFFFFF;\n        /* circle viz color (opacity is at 0.2) */\n        --circle-viz-color:#000000;\n        /* diameter of button, option can be changed by user */\n        --button-diameter:100px;\n        /* mic color*/\n        --mic-color:#FFFFFF;\n        /* text color, used for warning messaging*/\n        --text-color:#666;\n        \n        display: inline-block;\n        height: var(--button-diameter);\n        width: var(--button-diameter);\n      }\n\n      #paper-button{\n        outline:none;\n        background:var(--button-color);\n        border:none;\n        width:100%;\n        height:100%;\n        min-width:0;\n        border-radius:50%;\n        cursor:pointer;\n        transition: all ease 0.25s;\n        background-size: 100%;\n        background-repeat: no-repeat;\n        background-position: center;\n        box-shadow: 0 2px 5px 0 rgba(0,0,0,.26);\n        z-index:2;\n        position:absolute;\n        opacity:0.8;\n        margin:0;\n        left:0;\n      }\n\n      #paper-button.flat{\n        box-shadow:0px 0px 0px 0px;\n      }\n\n      #paper-button:hover{\n        box-shadow: 0 4px 7px 0 rgba(0,0,0,.20);\n      }\n\n      #paper-button.disabled{\n        box-shadow: 0px 0px 0px 0px;\n        cursor:not-allowed;\n        opacity:0.5;\n      }\n\n      #paper-button.listening, #paper-button.user-input{\n        box-shadow: 0px 0px 0px 0px;\n        opacity:1; \n        background:var(--active-color);\n      }\n\n      #paper-button.flat:hover{\n        box-shadow:0px 0px 0px 0px; \n      }\n\n      #paper-button.disabled:hover{\n        box-shadow: 0px 0px 0px 0px;\n      }\n\n      #circle-viz-container{\n        position:absolute;\n        z-index:0;\n        opacity:0;\n      }\n\n      .svg-container{\n        position:absolute;\n        top:0;\n        left:0;\n        width:100%;\n        height:100%;\n        background:rgba(255, 0, 0, 0);\n        z-index:3;\n        pointer-events: none;\n      }\n\n      #mic-svg, #mic-svg-disabled, #mic-loading{\n        height: 64%;\n        width:100%;\n        display: block;\n        position: absolute;\n        top: -11%;\n        left: 0px;\n        transform:translate(0, 50%);\n        z-index:3;\n        pointer-events:none;\n      }\n\n      #mic-loading {\n        opacity: 0;\n        top: -14%;\n        transform: translate(0, 50%) scale(0.8, 0.8);\n      }\n\n      #mic-loading circle:nth-child(1) {\n        animation: 0.7s micLoading 0s infinite;\n      }\n\n      #mic-loading circle:nth-child(2) {\n        animation: 0.7s micLoading 0.3s infinite;\n      }\n\n      #mic-loading circle:nth-child(3) {\n        animation: 0.7s micLoading 0.5s infinite;\n      }\n\n      @keyframes micLoading {\n        to {\n          fill-opacity: 0;\n        }\n        from {\n          fill-opacity: 1;\n        }\n      }\n\n\n      /* show the disabled mic when in disabled state */\n      .container #mic-svg-disabled{ display:none; }\n      .container.disabled #mic-svg-disabled{ display:block; }\n      .container.disabled #mic-svg{ display:none; }\n      .container.listening #mic-svg, .container.user-input #mic-svg { fill: #FFFFFF !important; }\n\n\n      .container{\n        position:absolute;\n        height: var(--button-diameter);\n        width: var(--button-diameter);\n      }\n\n      .circle{\n        width:calc(var(--button-diameter) - 4px);\n        height:calc(var(--button-diameter) - 4px);\n        background-color:var(--circle-viz-color);\n        opacity:0.2;\n        border-radius:50%;\n        position:absolute;\n        transform-origin: center center;\n        transform:scale(1);\n      }\n\n      #mic-not-allowed{\n        font-family: 'Noto Sans', sans-serif;\n        font-size: 1.0em;\n        -webkit-font-smoothing: antialiased;\n        color: var(--text-color);\n        width: 210px;\n        top: 100%;\n        display: none;\n        position: absolute;\n        margin-left: 0px;\n        transform: translateX(-50%);\n        left: 50%;\n        font-weight: 400;\n        text-align: center;\n        vertical-align: bottom;\n        margin-top: 15px;\n      }\n      #mic-not-allowed.show{\n        display:block;\n      }\n\n      paper-ripple{\n        position:absolute;\n      }\n\n    </style>\n\n\n    <!-- shadow DOM -->\n    <div class$=\"container [[state]]\">\n\n      <!-- rings that scale out from button with user input (only available with autodetect)  -->\n      <div id=\"circle-viz-container\"></div>\n      \n      <!-- clickable button -->\n      <button id=\"paper-button\" class$=\"voice-button [[state]] [[flatStr]] [[autoDetectStr]]\"><paper-ripple></paper-ripple></button>\n      \n      <div class=\"svg-container\">\n        <!-- mic icon -->\n        <svg id=\"mic-svg\" xmlns=\"http://www.w3.org/2000/svg\" width=\"100%\" height=\"100%\" viewBox=\"0 0 24 24\" fill=\"#000000\">\n            <path d=\"M12 14c1.66 0 2.99-1.34 2.99-3L15 5c0-1.66-1.34-3-3-3S9 3.34 9 5v6c0 1.66 1.34 3 3 3zm5.3-3c0 3-2.54 5.1-5.3 5.1S6.7 14 6.7 11H5c0 3.41 2.72 6.23 6 6.72V21h2v-3.28c3.28-.48 6-3.3 6-6.72h-1.7z\"></path>\n            <path d=\"M0 0h24v24H0z\" fill=\"none\"></path>\n        </svg>\n        <svg id=\"mic-svg-disabled\" xmlns=\"http://www.w3.org/2000/svg\" width=\"100%\" height=\"100%\" viewBox=\"0 0 24 24\" fill=\"#000000\">\n            <path d=\"M0 0h24v24H0zm0 0h24v24H0z\" fill=\"none\"></path>\n            <path d=\"M19 11h-1.7c0 .74-.16 1.43-.43 2.05l1.23 1.23c.56-.98.9-2.09.9-3.28zm-4.02.17c0-.06.02-.11.02-.17V5c0-1.66-1.34-3-3-3S9 3.34 9 5v.18l5.98 5.99zM4.27 3L3 4.27l6.01 6.01V11c0 1.66 1.33 3 2.99 3 .22 0 .44-.03.65-.08l1.66 1.66c-.71.33-1.5.52-2.31.52-2.76 0-5.3-2.1-5.3-5.1H5c0 3.41 2.72 6.23 6 6.72V21h2v-3.28c.91-.13 1.77-.45 2.54-.9L19.73 21 21 19.73 4.27 3z\"></path>\n        </svg>\n      <svg id=\"mic-loading\" version=\"1.1\" xmlns=\"http://www.w3.org/2000/svg\" x=\"0px\" y=\"0px\" viewBox=\"0 0 727 353\" fill=\"#ffffff\">\n        <circle cx=\"136.5\" cy=\"176.5\" r=\"73.5\"></circle>\n        <circle cx=\"363.5\" cy=\"176.5\" r=\"73.5\"></circle>\n        <circle cx=\"590.5\" cy=\"176.5\" r=\"73.5\"></circle>\n      </svg>\n      </div>\n\n      <div id=\"mic-not-allowed\">Cannot access microphone</div>\n    \n    </div>\n\n  </template>\n\n  \n\n</dom-module>";document.head.appendChild($_documentContainer.content);// Copyright 2017 Google Inc.
//
...
window.AudioContext=window.AudioContext||window.webkitAudioContext;function VoiceRecorder(autodetect,onStopRecordComplete,timeoutval){var _this33=this;this.autodetect=autodetect;this.onStopRecordComplete=onStopRecordComplete;this.microphone;this.isRecording=!1;this.isSpeaking=!1;this.audioContext;this.leftChannel=[];this.recordingLength=0;this.processor;this.timeoutval=timeoutval;this.config={sampleRate:44100,bufferLen:4096,numChannels:1,mimeType:"audio/mpeg"};this.analyzer=function(context){var listener=context.createAnalyser();_this33.microphone.connect(listener);listener.fftSize=256;var bufferLength=listener.frequencyBinCount,analyserData=new Uint8Array(bufferLength)};this.onAudioProcess=function(e){var left=e.inputBuffer.getChannelData(0);_this33.recordingLength+=_this33.config.bufferLen;// we clone the samples
_this33.leftChannel.push(new Float32Array(left));// Check for pause while speaking
var MIN_SPEAKING_VOLUME=.01,sum=0,i,clipcount=0;for(i=0;i<left.length;++i){sum+=left[i]*left[i];if(.99<Math.abs(left[i])){clipcount+=1}}var volume=Math.sqrt(sum/left.length);if(volume>MIN_SPEAKING_VOLUME){_this33.isSpeaking=!0;clearTimeout(_this33.speakingTimeout)}else{if(_this33.isSpeaking){_this33.isSpeaking=!1;clearTimeout(_this33.speakingTimeout);console.log("adding timeout of ",_this33.timeoutval,_this33.assessmentToken);_this33.speakingTimeout=setTimeout(function(){_this33.stopRecord()},_this33.timeoutval)}}};this.logError=function(error){console.error(error)};this.startRecord=function(){_this33.audioContext=new AudioContext;/** 
                                             * Create a ScriptProcessorNode
                                             * */if(_this33.audioContext.createJavaScriptNode){_this33.processor=_this33.audioContext.createJavaScriptNode(_this33.config.bufferLen,_this33.config.numChannels,_this33.config.numChannels)}else if(_this33.audioContext.createScriptProcessor){_this33.processor=_this33.audioContext.createScriptProcessor(_this33.config.bufferLen,_this33.config.numChannels,_this33.config.numChannels)}else{console.log("WebAudio API has no support on this browser.")}_this33.processor.connect(_this33.audioContext.destination);/**
                                                            *  ask permission of the user for use this.microphone or camera  
                                                            * */navigator.mediaDevices.getUserMedia({audio:!0,video:!1}).then(_this33.gotStreamMethod.bind(_this33)).catch(_this33.logError)};this.getBuffers=function(event){for(var buffers=[],ch=0;2>ch;++ch){buffers[ch]=event.inputBuffer.getChannelData(ch)}return buffers};this.gotStreamMethod=function(stream){// audioElement.src = "";
_this33.isRecording=!0;_this33.tracks=stream.getTracks();/** 
                                       * Create a MediaStreamAudioSourceNode for the this.microphone 
                                       * */_this33.microphone=_this33.audioContext.createMediaStreamSource(stream);/** 
                                                                          * connect the AudioBufferSourceNode to the gainNode 
                                                                          * */_this33.microphone.connect(_this33.processor);// encoder = new Mp3LameEncoder(audioContext.sampleRate, 160);
/** 
     * Give the node a function to process audio events 
     */if(_this33.autodetect){_this33.processor.onaudioprocess=_this33.onAudioProcess}_this33.analyzer(_this33.audioContext)};this.clearRecordedData=function(){_this33.recordingLength=0;_this33.leftChannel=[]};this.stopRecord=function(){var callback=_this33.onStopRecordComplete;_this33.isRecording=!1;_this33.audioContext.close();_this33.processor.disconnect();_this33.tracks.forEach(function(track){return track.stop()});_this33.mergeLeftRightBuffers({sampleRate:_this33.config.sampleRate,numberOfAudioChannels:_this33.config.numChannels,internalInterleavedLength:_this33.recordingLength,leftBuffers:_this33.leftChannel,rightBuffers:1===_this33.config.numChannels?[]:rightChannel},function(buffer,view){self.blob=new Blob([view],{type:"audio/wav"});self.buffer=new ArrayBuffer(view.buffer.byteLength);self.view=view;self.sampleRate=_this33.config.sampleRate;self.bufferSize=_this33.config.bufferLen;self.length=_this33.recordingLength;callback&&callback(self.blob);_this33.clearRecordedData();isAudioProcessStarted=!1})};this.mergeLeftRightBuffers=function(config,callback){function mergeAudioBuffers(config,cb){var numberOfAudioChannels=config.numberOfAudioChannels,leftBuffers=config.leftBuffers.slice(0),rightBuffers=config.rightBuffers.slice(0),sampleRate=config.sampleRate,internalInterleavedLength=config.internalInterleavedLength,desiredSampRate=config.desiredSampRate;// todo: "slice(0)" --- is it causes loop? Should be removed?
if(2===numberOfAudioChannels){leftBuffers=mergeBuffers(leftBuffers,internalInterleavedLength);rightBuffers=mergeBuffers(rightBuffers,internalInterleavedLength);if(desiredSampRate){leftBuffers=interpolateArray(leftBuffers,desiredSampRate,sampleRate);rightBuffers=interpolateArray(rightBuffers,desiredSampRate,sampleRate)}}if(1===numberOfAudioChannels){leftBuffers=mergeBuffers(leftBuffers,internalInterleavedLength);if(desiredSampRate){leftBuffers=interpolateArray(leftBuffers,desiredSampRate,sampleRate)}}// set sample rate as desired sample rate
if(desiredSampRate){sampleRate=desiredSampRate}// for changing the sampling rate, reference:
// http://stackoverflow.com/a/28977136/552182
function interpolateArray(data,newSampleRate,oldSampleRate){var fitCount=Math.round(data.length*(newSampleRate/oldSampleRate)),newData=[],springFactor=+((data.length-1)/(fitCount-1));//var newData = new Array();
newData[0]=data[0];// for new allocation
for(var i=1;i<fitCount-1;i++){var tmp=i*springFactor,before=(+Math.floor(tmp)).toFixed(),after=(+Math.ceil(tmp)).toFixed(),atPoint=tmp-before;//var before = new Number(Math.floor(tmp)).toFixed();
//var after = new Number(Math.ceil(tmp)).toFixed();
newData[i]=linearInterpolate(data[before],data[after],atPoint)}newData[fitCount-1]=data[data.length-1];// for new allocation
return newData}function linearInterpolate(before,after,atPoint){return before+(after-before)*atPoint}function mergeBuffers(channelBuffer,rLength){for(var result=new Float64Array(rLength),offset=0,lng=channelBuffer.length,i=0,buffer;i<lng;i++){buffer=channelBuffer[i];result.set(buffer,offset);offset+=buffer.length}return result}function interleave(leftChannel,rightChannel){for(var length=leftChannel.length+rightChannel.length,result=new Float64Array(length),inputIndex=0,index=0;index<length;){result[index++]=leftChannel[inputIndex];result[index++]=rightChannel[inputIndex];inputIndex++}return result}function writeUTFBytes(view,offset,string){for(var lng=string.length,i=0;i<lng;i++){view.setUint8(offset+i,string.charCodeAt(i))}}// interleave both channels together
var interleaved;if(2===numberOfAudioChannels){interleaved=interleave(leftBuffers,rightBuffers)}if(1===numberOfAudioChannels){interleaved=leftBuffers}var interleavedLength=interleaved.length,resultingBufferLength=44+2*interleavedLength,buffer=new ArrayBuffer(resultingBufferLength),view=new DataView(buffer);// create wav file
// RIFF chunk descriptor/identifier
writeUTFBytes(view,0,"RIFF");// RIFF chunk length
view.setUint32(4,44+2*interleavedLength,!0);// RIFF type
writeUTFBytes(view,8,"WAVE");// format chunk identifier
// FMT sub-chunk
writeUTFBytes(view,12,"fmt ");// format chunk length
view.setUint32(16,16,!0);// sample format (raw)
view.setUint16(20,1,!0);// stereo (2 channels)
view.setUint16(22,numberOfAudioChannels,!0);// sample rate
view.setUint32(24,sampleRate,!0);// byte rate (sample rate * block align)
view.setUint32(28,2*sampleRate,!0);// block align (channel count * bytes per sample)
view.setUint16(32,2*numberOfAudioChannels,!0);// bits per sample
view.setUint16(34,16,!0);// data sub-chunk
// data chunk identifier
writeUTFBytes(view,36,"data");// data chunk length
view.setUint32(40,2*interleavedLength,!0);// write the PCM samples
for(var lng=interleavedLength,index=44,volume=1,i=0;i<lng;i++){view.setInt16(index,interleaved[i]*(32767*volume),!0);index+=2}if(cb){return cb({buffer:buffer,view:view})}postMessage({buffer:buffer,view:view})}var webWorker=_this33.processInWebWorker(mergeAudioBuffers);webWorker.onmessage=function(event){callback(event.data.buffer,event.data.view);// release memory
URL.revokeObjectURL(webWorker.workerURL)};webWorker.postMessage(config)};this.processInWebWorker=function(_function){var workerURL=URL.createObjectURL(new Blob([_function.toString(),";this.onmessage =  function (e) {"+_function.name+"(e.data);}"],{type:"application/javascript"})),worker=new Worker(workerURL);worker.workerURL=workerURL;return worker}}// Copyright 2017 Google Inc.
//
//     Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
//     You may obtain a copy of the License at
...
*/function CircleViz(containerEl,onNoMicAccess){var _this34=this;this.containerEl=containerEl;this.stopped=!0;this.circle=document.createElement("div");this.circle.classList.add("circle");this.containerEl.appendChild(this.circle);this.isMobile=window.mobileAndTabletcheck();this.onNoMicAccess=onNoMicAccess;this.attachedUserMediaStream=!1;// create one audio context
var AudioContext=window.AudioContext||window.webkitAudioContext;if("undefined"!==typeof AudioContext){this.audioContext=new AudioContext}this.attachUserMediaStream=function(){// if it has been stopped, let's open it back up
if(!_this34.attachedUserMediaStream||_this34.audioStream&&!_this34.audioStream.active){if(_this34.isMobile){_this34.doDraw()}else{if(navigator.mediaDevices&&navigator.mediaDevices.getUserMedia){navigator.mediaDevices.getUserMedia({audio:!0}).then(_this34.soundAllowed.bind(_this34)).catch(_this34.soundNotAllowed.bind(_this34))}else{// deprecated approach: https://developer.mozilla.org/en-US/docs/Web/API/Navigator/getUserMedia
navigator.getUserMedia=navigator.getUserMedia||navigator.webkitGetUserMedia||navigator.mozGetUserMedia||null;if(navigator.getUserMedia)navigator.getUserMedia({audio:!0},function(stream){return _this34.soundAllowed(stream)},function(e){return _this34.soundNotAllowed(e)})}}_this34.attachedUserMediaStream=!0}};this.soundNotAllowed=function(e){// circle viz graceful degrade, but let know main component know
_this34.onNoMicAccess()};this.soundAllowed=function(stream){_this34.audioStream=stream;if(_this34.audioContext){var audioStream=_this34.audioContext.createMediaStreamSource(stream);_this34.analyser=_this34.audioContext.createAnalyser();audioStream.connect(_this34.analyser);_this34.analyser.fftSize=1024;_this34.frequencyArray=new Uint8Array(_this34.analyser.frequencyBinCount)}_this34.doDraw()};this.getCircleScale=function(){var scale=1;if(_this34.analyser){_this34.analyser.getByteFrequencyData(_this34.frequencyArray);// todo: frequency array is all zeros for android chrome
for(var freqSum=0,_i63=0;255>_i63;_i63++){freqSum+=_this34.frequencyArray[_i63]}var freqAvg=freqSum/255;scale=1+1.2*(freqAvg/255)}else{// don't have FFT spectrum, so fake it
var currScale=_this34.getCurrentScale();if(currScale){var rand=1+Math.random();// smooth it
scale=currScale+(rand-currScale)/8}}return scale};this.doDraw=function(){if(!_this34.stopped){var newScale=_this34.getCircleScale();_this34.circle.style.transform="scale("+newScale+") translate(2px, 2px)"}else{// stopped, slowly scale down
var currScale=_this34.getCurrentScale();if(currScale){if(0<currScale){currScale=Math.max(0,currScale-.05);_this34.circle.style.transform="scale("+currScale+") translate(2px, 2px)"}}}_this34.animation=window.requestAnimationFrame(_this34.doDraw.bind(_this34))};this.getCurrentScale=function(){var transformProp=_this34.circle.style.transform&&""!==_this34.circle.style.transform?_this34.circle.style.transform:"scale(1)",splitProperty=transformProp.split(new RegExp(/\(|\)/,"g"));if(2<splitProperty.length){var currScale=parseFloat(splitProperty[1]);return currScale}return null};this.startAnimation=function(){if(_this34.stopped){_this34.containerEl.style.opacity=1;_this34.stopped=!1;_this34.attachUserMediaStream()}};this.stopAnimation=function(){_this34.stopped=!0;_this34.containerEl.style.opacity=0;if(_this34.audioStream&&0<_this34.audioStream.getAudioTracks().length){_this34.audioStream.getAudioTracks()[0].stop()}}}window.componentTranslations={micNotAllowed:{af:"Kan nie toegang tot mikrofoon h\xEA nie",sq:"Nuk mund t\xEB hyni n\xEB mikrofon",am:"\u121B\u12ED\u12AD\u122E\u134E\u1295 \u1218\u12F5\u1228\u1235 \u12A0\u120D\u1270\u127B\u1208\u121D",ar:"\u0644\u0627 \u064A\u0645\u0643\u0646 \u0627\u0644\u062F\u062E\u0648\u0644 \u0625\u0644\u0649 \u0627\u0644\u0645\u064A\u0643\u0631\u0648\u0641\u0648\u0646",hy:"\u0540\u0576\u0561\u0580\u0561\u057E\u0578\u0580 \u0579\u0567 \u0574\u0578\u0582\u057F\u0584 \u0563\u0578\u0580\u056E\u0565\u056C \u056D\u0578\u057D\u0561\u0583\u0578\u0572",az:"Mikrofona daxil ola bilmir",eu:"Ezin da mikrofonoa atzitu",be:"\u041D\u0435 \u045E\u0434\u0430\u0435\u0446\u0446\u0430 \u0430\u0442\u0440\u044B\u043C\u0430\u0446\u044C \u0434\u043E\u0441\u0442\u0443\u043F \u0434\u0430 \u043C\u0456\u043A\u0440\u0430\u0444\u043E\u043D\u0443",bs:"Ne mogu pristupiti mikrofonu",bg:"\u041D\u044F\u043C\u0430 \u0434\u043E\u0441\u0442\u044A\u043F \u0434\u043E \u043C\u0438\u043A\u0440\u043E\u0444\u043E\u043D\u0430",ca:"No es pot accedir al micr\xF2fon",ceb:"Dili maka-access sa mikropono",ny:"Sangathe kulumikiza maikolofoni",zh:"\u65E0\u6CD5\u8BBF\u95EE\u9EA6\u514B\u98CE","zh-TW":"\u7121\u6CD5\u8A2A\u554F\u9EA5\u514B\u98A8",co:"\xD9n pudete micca accede \xE0 u micrufonu",hr:"Nije mogu\u0107e pristupiti mikrofonu",cs:"Nem\xE1 p\u0159\xEDstup k mikrofonu",da:"Kan ikke f\xE5 adgang til mikrofonen",nl:"Kan geen microfoon openen",en:"Cannot access microphone",eo:"Ne povas aliri mikrofonon",et:"Ei saa mikrofoni juurde p\xE4\xE4seda",tl:"Hindi ma-access ang mikropono",fi:"Mikrofonia ei voi k\xE4ytt\xE4\xE4",fr:"Impossible d&#39;acc\xE9der au microphone",fy:"Kin gjin mikrofoan tagong krije",gl:"Non se pode acceder ao micr\xF3fono",ka:"\u10DB\u10D8\u10D9\u10E0\u10DD\u10E4\u10DD\u10DC\u10D8 \u10D5\u10D4\u10E0 \u10EC\u10D5\u10D3\u10DD\u10DB\u10D0",de:"Kann nicht auf Mikrofon zugreifen",el:"\u0394\u03B5\u03BD \u03B5\u03AF\u03BD\u03B1\u03B9 \u03B4\u03C5\u03BD\u03B1\u03C4\u03AE \u03B7 \u03C0\u03C1\u03CC\u03C3\u03B2\u03B1\u03C3\u03B7 \u03C3\u03C4\u03BF \u03BC\u03B9\u03BA\u03C1\u03CC\u03C6\u03C9\u03BD\u03BF",gu:"\u0AAE\u0ABE\u0A87\u0A95\u0ACD\u0AB0\u0ACB\u0AAB\u0ACB\u0AA8\u0AA8\u0AC7 \u0A8D\u0A95\u0ACD\u0AB8\u0AC7\u0AB8 \u0A95\u0AB0\u0AC0 \u0AB6\u0A95\u0AA4\u0ABE \u0AA8\u0AA5\u0AC0",ht:"Pa ka jwenn mikwof\xF2n",ha:"Ba za a iya shiga makirufo ba",haw:"\u02BBA\u02BBole hiki ke ho\u02BBokomo i ka microphone",iw:"\u05DC\u05D0 \u05E0\u05D9\u05EA\u05DF \u05DC\u05D2\u05E9\u05EA \u05DC\u05DE\u05D9\u05E7\u05E8\u05D5\u05E4\u05D5\u05DF",hi:"\u092E\u093E\u0907\u0915\u094D\u0930\u094B\u092B\u093C\u094B\u0928 \u0924\u0915 \u0928\u0939\u0940\u0902 \u092A\u0939\u0941\u0902\u091A \u0938\u0915\u0924\u0947",hmn:"Mus saib tsis tau microphone",hu:"Nem lehet hozz\xE1f\xE9rni a mikrofonhoz",is:"Get ekki fengi\xF0 a\xF0gang a\xF0 hlj\xF3\xF0nemanum",ig:"Enwegh\u1ECB ike \u1ECBnweta igwe okwu",id:"Tidak dapat mengakses mikrofon",ga:"N\xED f\xE9idir rochtain a fh\xE1il ar mhicreaf\xF3n",it:"Impossibile accedere al microfono",ja:"\u30DE\u30A4\u30AF\u306B\u30A2\u30AF\u30BB\u30B9\u3067\u304D\u306A\u3044",jw:"Ora bisa ngakses mikropon",kn:"\u0CAE\u0CC8\u0C95\u0CCD\u0CB0\u0CCA\u0CAB\u0CCB\u0CA8\u0CCD \u0CAA\u0CCD\u0CB0\u0CB5\u0CC7\u0CB6\u0CBF\u0CB8\u0CB2\u0CC1 \u0CB8\u0CBE\u0CA7\u0CCD\u0CAF\u0CB5\u0CBF\u0CB2\u0CCD\u0CB2",kk:"\u041C\u0438\u043A\u0440\u043E\u0444\u043E\u043D\u0493\u0430 \u043A\u0456\u0440\u0443 \u043C\u04AF\u043C\u043A\u0456\u043D \u0435\u043C\u0435\u0441",km:"\u1798\u17B7\u1793\u17A2\u17B6\u1785\u1785\u17BC\u179B\u178A\u17C6\u178E\u17BE\u179A\u1780\u17B6\u179A\u1798\u17B8\u1780\u17D2\u179A\u17BC\u17A0\u17D2\u179C\u17BC\u1793",ko:"\uB9C8\uC774\uD06C\uC5D0 \uC561\uC138\uC2A4 \uD560 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4.",ku:"Mirov dikare m\xEEkrofon nikare",ky:"\u043A\u043E\u0435\u043B\u0443 \u043A\u0438\u0440\u0435 \u0430\u043B\u0431\u0430\u0439\u0442",lo:"\u0E9A\u0ECD\u0EC8\u0EAA\u0EB2\u0EA1\u0EB2\u0E94\u0EC0\u0E82\u0EBB\u0EC9\u0EB2\u0EC0\u0E96\u0EB4\u0E87\u0EC4\u0EA1\u0EC2\u0E84\u0EC2\u0E9F\u0E99",la:"Access potest non tortor ligula, facilisis",lv:"Nevar piek\u013C\u016Bt mikrofonam",lt:"Negalite pasiekti mikrofono",lb:"Kann net op den Mikrofon goen",mk:"\u041D\u0435 \u043C\u043E\u0436\u0430\u043C \u0434\u0430 \u043F\u0440\u0438\u0441\u0442\u0430\u043F\u0430\u043C \u043D\u0430 \u043C\u0438\u043A\u0440\u043E\u0444\u043E\u043D",mg:"Tsy afaka mikitika mikr\xF4fo",ms:"Tidak boleh mengakses mikrofon",ml:"\u0D2E\u0D48\u0D15\u0D4D\u0D30\u0D4B\u0D2B\u0D4B\u0D7A \u0D06\u0D15\u0D4D\u0D38\u0D38\u0D4D\u0D38\u0D41\u0D1A\u0D46\u0D2F\u0D4D\u0D2F\u0D3E\u0D28\u0D3E\u0D35\u0D3F\u0D32\u0D4D\u0D32",mt:"Ma tistax ta\u010B\u010Bessa mikrofonu",mi:"Kaore e taea te uru ki te hopuorooro",mn:"\u041C\u0438\u043A\u0440\u043E\u0444\u043E\u043D \u0440\u0443\u0443 \u043D\u044D\u0432\u0442\u044D\u0440\u0447 \u0447\u0430\u0434\u0430\u0445\u0433\u04AF\u0439 \u0431\u0430\u0439\u043D\u0430",my:"\u1019\u102D\u102F\u1000\u103A\u1001\u101B\u102D\u102F\u1016\u102F\u1014\u103A\u1038\u1000\u102D\u102F\u101B\u101A\u1030\u1019\u1014\u102D\u102F\u1004\u103A",no:"Kan ikke f\xE5 tilgang til mikrofonen",ps:"\u0645\u0627\u06CC\u06A9\u0631\u0648\u0641\u0648\u0646 \u062A\u0647 \u0644\u0627\u0633\u0631\u0633\u06CC \u0646\u0634\u064A \u06A9\u0648\u0644\u06CC",fa:"\u0645\u06CC \u062A\u0648\u0627\u0646\u06CC\u062F \u0628\u0647 \u0645\u06CC\u06A9\u0631\u0648\u0641\u0648\u0646 \u062F\u0633\u062A\u0631\u0633\u06CC \u067E\u06CC\u062F\u0627 \u06A9\u0646\u06CC\u062F",pl:"Nie mo\u017Cna uzyska\u0107 dost\u0119pu do mikrofonu",pt:"N\xE3o \xE9 poss\xEDvel acessar o microfone",pa:"\u0A2E\u0A3E\u0A08\u0A15\u0A4D\u0A30\u0A4B\u0A2B\u0A4B\u0A28 \u0A24\u0A71\u0A15 \u0A2A\u0A39\u0A41\u0A70\u0A1A \u0A28\u0A39\u0A40\u0A02 \u0A15\u0A30 \u0A38\u0A15\u0A26\u0A3E",ro:"Nu pute\u021Bi accesa microfonul",ru:"\u041D\u0435 \u0443\u0434\u0430\u0435\u0442\u0441\u044F \u043F\u043E\u043B\u0443\u0447\u0438\u0442\u044C \u0434\u043E\u0441\u0442\u0443\u043F \u043A \u043C\u0438\u043A\u0440\u043E\u0444\u043E\u043D\u0443",sm:"E le mafai ona maua le masini faaleotele leo",gd:"Cha ghabh mi lorg air microf\xF2n",sr:"\u041D\u0435 \u043C\u043E\u0433\u0443 \u043F\u0440\u0438\u0441\u0442\u0443\u043F\u0438\u0442\u0438 \u043C\u0438\u043A\u0440\u043E\u0444\u043E\u043D\u0443",st:"Ha e khone ho finyella microphone",sn:"Haikwanise kuwana maikorofoni",sd:"\u0645\u0627\u0626\u06AA\u0631\u0648\u0641\u0648\u0646 \u062A\u0627\u0626\u064A\u0646 \u0631\u0633\u0627\u0626\u064A \u0646\u0647 \u067F\u064A \u0633\u06AF\u06BE\u064A",si:"\u0DB8\u0DBA\u0DD2\u0D9A\u0DCA\u0DBB\u0DC6\u0DDD\u0DB1\u0DBA \u0DC0\u0DD9\u0DAD \u0DB4\u0DD2\u0DC0\u0DD2\u0DC3\u0DD2\u0DBA \u0DB1\u0DDC\u0DC4\u0DD0\u0D9A",sk:"Nem\xE1te pr\xEDstup k mikrof\xF3nu",sl:"Ne morem dostopati do mikrofona",so:"Ma heli kartid makarafoon",es:"No se puede acceder al micr\xF3fono",su:"moal bisa ngakses mikropon",sw:"Haiwezi kufikia kipaza sauti",sv:"Kan inte komma \xE5t mikrofonen",tg:"\u041C\u0438\u043A\u0440\u043E\u0444\u043E\u043D\u0440\u043E \u0431\u0430 \u043A\u043E\u0440 \u0430\u043D\u0434\u043E\u0445\u0442\u0430 \u043D\u0430\u043C\u0435\u0448\u0430\u0432\u0430\u0434",ta:"\u0BAE\u0BC8\u0B95\u0BCD\u0BB0\u0BCB\u0B83\u0BAA\u0BCB\u0BA9\u0BC8 \u0B85\u0BA3\u0BC1\u0B95 \u0BAE\u0BC1\u0B9F\u0BBF\u0BAF\u0BB5\u0BBF\u0BB2\u0BCD\u0BB2\u0BC8",te:"\u0C2E\u0C48\u0C15\u0C4D\u0C30\u0C4B\u0C2B\u0C4B\u0C28\u0C4D\u0C28\u0C41 \u0C2A\u0C4D\u0C30\u0C3E\u0C2A\u0C4D\u0C2F\u0C24 \u0C1A\u0C47\u0C2F\u0C32\u0C47\u0C30\u0C41",th:"\u0E44\u0E21\u0E48\u0E2A\u0E32\u0E21\u0E32\u0E23\u0E16\u0E40\u0E02\u0E49\u0E32\u0E16\u0E36\u0E07\u0E44\u0E21\u0E42\u0E04\u0E23\u0E42\u0E1F\u0E19\u0E44\u0E14\u0E49",tr:"Mikrofona eri\u015Filemiyor",uk:"\u041D\u0435 \u0432\u0434\u0430\u0454\u0442\u044C\u0441\u044F \u043E\u0442\u0440\u0438\u043C\u0430\u0442\u0438 \u0434\u043E\u0441\u0442\u0443\u043F \u0434\u043E \u043C\u0456\u043A\u0440\u043E\u0444\u043E\u043D\u0430",ur:"\u0645\u0627\u0626\u06CC\u06A9\u0631\u0648\u0641\u0648\u0646 \u062A\u06A9 \u0631\u0633\u0627\u0626\u06CC \u062D\u0627\u0635\u0644 \u0646\u06C1\u06CC\u06BA",uz:"Mikrofonga kirib bo&#39;lmadi",vi:"Kh\xF4ng th\u1EC3 truy c\u1EADp micr\xF4",cy:"Methu \xE2 chael mynediad i feicroffon",xh:"Ayikwazi ukufikelela kwimakrofoni"/* 
                                                Note: deprecation warning for styling master document doesn't apply within <template> tags
                                                Read more: https://www.polymer-project.org/blog/2017-10-18-upcoming-changes
                                                 */ /*
                                                    ...
                                                    */}};var ListeningState=Object.freeze({IDLE:"idle",LISTENING:"listening",USER_INPUT:"user-input",DISABLED:"disabled"}),KeyboardTriggers=Object.freeze({ALL_KEYS:"all-keys",SPACE_BAR:"space-bar",NONE:"none"}),audioStream,VoiceButton=/*#__PURE__*/function(_PolymerElement2){babelHelpers.inherits(VoiceButton,_PolymerElement2);babelHelpers.createClass(VoiceButton,null,[{key:"is",get:function get(){return"voice-button"}},{key:"properties",get:function get(){return{flat:{type:Boolean,reflectToAttribute:!0,value:!1},autodetect:{type:Boolean,notify:!1,reflectToAttribute:!0,value:!1},clickForPermission:{type:Boolean,notify:!1,reflectToAttribute:!0,value:!1},hidePermissionText:{type:Boolean,notify:!1,reflectToAttribute:!0,value:!1},language:{type:String,reflectToAttribute:!0,value:"en-US"},disabled:{type:Boolean,notify:!0,reflectToAttribute:!0,value:!1,observer:"_onEnabledStateChange"},keyboardTrigger:{type:String,reflectToAttribute:!0,value:KeyboardTriggers.SPACE_BAR},pressed:{type:Boolean,notify:!1,reflectToAttribute:!0,readOnly:!0,value:!1},cloudSpeechApiKey:{type:String,readOnly:!1,reflectToAttribute:!1,value:null},assessmentToken:{type:String,readOnly:!1,reflectToAttribute:!1,value:null},reassess:{type:String,readOnly:!1,reflectToAttribute:!1,value:"false"},scoreidx:{type:Number,readOnly:!1,reflectToAttribute:!1,value:0},timeoutval:{type:Number,readOnly:!1,reflectToAttribute:!1,value:500},cloudSpeechUrl:{type:String,readOnly:!1,reflectToAttribute:!1,value:null},recordingToken:{type:String,readOnly:!1,reflectToAttribute:!1,value:null},lesson:{type:Number,readOnly:!1,reflectToAttribute:!1,value:null},alternatives:{type:Number,readOnly:!1,reflectToAttribute:!1,value:null},context:{type:String,readOnly:!1,reflectToAttribute:!1,value:null},state:{type:String,notify:!1,readOnly:!0,reflectToAttribute:!0,value:ListeningState.IDLE,observer:"_onStateChange"},supported:{type:Boolean,readOnly:!0,reflectToAttribute:!0,value:!0},flatStr:String,autoDetectStr:String}}}]);function VoiceButton(){var _this35;babelHelpers.classCallCheck(this,VoiceButton);_this35=babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(VoiceButton).call(this));_this35._setState(ListeningState.IDLE);return _this35}babelHelpers.createClass(VoiceButton,[{key:"ready",value:function ready(){var _this36=this;babelHelpers.get(babelHelpers.getPrototypeOf(VoiceButton.prototype),"ready",this).call(this);this.useCloudSpeech=!1;// warn host if a cloud speech api isn't provided
if("string"!==typeof this.cloudSpeechApiKey){console.warn("For fallback support, it is recommended to provide a cloud-speech-api-key.")}// Get the SpeechRecognition object
...
// Opera 8.0+
var isOpera=!!window.opr&&!!opr.addons||!!window.opera||0<=navigator.userAgent.indexOf(" OPR/"),SpeechRecognition=window.webkitSpeechRecognition;// if (typeof SpeechRecognition === 'undefined' || isOpera) {
if(!0){// make sure you have a Cloud Speech API key 
if("string"!==typeof this.cloudSpeechApiKey){console.error("Need to provide a cloud-speech-api-key.");this._setState(ListeningState.DISABLED);this.set("disabled",!0);return}// check for WebRTC support
...
if(this.checkWebRTC()){this.useCloudSpeech=!0}else{this._setSupported(!1);// disable button if speech recognition isn't available
this._setState(ListeningState.DISABLED);this.set("disabled",!0);console.error("WebRTC is not supported on this browser.");return}}this.set("flatStr",this.flat?"flat":"");this.set("autoDetectStr",this.autodetect?"auto-detect":"");var micColor=getComputedStyle(this).getPropertyValue("--mic-color");this.googleMicEl=this.$["mic-svg"];this.googleMicLoaderEl=this.$["mic-loading"];this.googleMicDisabledEl=this.$["mic-svg-disabled"];this.buttonEl=this.$["paper-button"];var circleVizContainer=this.$["circle-viz-container"];this.circleViz=new CircleViz(circleVizContainer,function(){_this36.onSpeechError({error:"not-allowed"})});// if user wants to ask for permission when clicked, then wait
if(!this.clickForPermission){this.circleViz.attachUserMediaStream()}// set mic color
this.googleMicEl.style.fill=this.googleMicDisabledEl.style.fill=micColor;// mouse listeners
this.buttonEl.addEventListener("mousedown",this.mouseDown.bind(this));this.buttonEl.addEventListener("mouseup",this.mouseUp.bind(this));// listen for space bar on document
if(this.keyboardTrigger!==KeyboardTriggers.NONE){document.body.addEventListener("keydown",this.keyDown.bind(this));document.body.addEventListener("keyup",this.keyUp.bind(this))}if(this.hidePermissionText){this.$["mic-not-allowed"].style.display="none"}// setup microphone-not-allowed messaging
var translatedCopy=this.getTranslation();if(translatedCopy){this.$["mic-not-allowed"].innerHTML=translatedCopy}if(!this.useCloudSpeech){// set up speech recognition
this.recognition=new SpeechRecognition;if(""!=this.language)this.recognition.lang=this.language;// weird issue where `interimResults = true` is cutting off speech 
...
var isMobile=window.mobileAndTabletcheck();this.recognition.interimResults=!isMobile;this.recognition.continuous=isMobile?!1:!this.autodetect;this.recognition.maxAlternatives=0;// add event listeners
this.recognition.onresult=this.onSpeechResult.bind(this);this.recognition.onspeechend=this.onSpeechEnd.bind(this);this.recognition.onerror=this.onSpeechError.bind(this)}else{// get blob data to send up to Cloud Speech API
this.voiceRecorder=new VoiceRecorder(this.autodetect,this.onStopRecordingVoiceComplete.bind(this),this.timeoutval)}}},{key:"getTranslation",value:function getTranslation(){var full=window.componentTranslations.micNotAllowed[this.language];if("undefined"!==typeof full)return full;var half=window.componentTranslations.micNotAllowed[this.language.split("-")[0].toLowerCase()];if("undefined"!==typeof half)return half;return null}},{key:"keyDown",value:function keyDown(event){if(this.state===ListeningState.DISABLED)return;this.pressed=!0;if(32==event.keyCode&&this.keyboardTrigger===KeyboardTriggers.SPACE_BAR||this.keyboardTrigger===KeyboardTriggers.ALL_KEYS){if(this.state!==ListeningState.USER_INPUT&&this.state!==ListeningState.LISTENING){this.startListening()}}}},{key:"keyUp",value:function keyUp(event){if(this.state===ListeningState.DISABLED)return;this.pressed=!1;if(32==event.keyCode&&this.keyboardTrigger===KeyboardTriggers.SPACE_BAR||this.keyboardTrigger===KeyboardTriggers.ALL_KEYS){if(this.autodetect&&(this.state===ListeningState.IDLE||this.state===ListeningState.USER_INPUT)){this.stopListening()}else if(!this.autodetect){this.stopListening()}}}},{key:"mouseDown",value:function mouseDown(event){// If we use Cloud Speech, we'll have it start and stop by a toggle
if(this.state===ListeningState.DISABLED)return;this.pressed=!0;if(this.state!==ListeningState.USER_INPUT&&this.state!==ListeningState.LISTENING){this.startListening()}else if(this.state===ListeningState.LISTENING){this.stopListening()}}},{key:"mouseUp",value:function mouseUp(event){if(this.state===ListeningState.DISABLED)return;// if in autodetect mode but user taps again to stop
if(this.autodetect&&(this.state===ListeningState.IDLE||this.state===ListeningState.USER_INPUT)){this.stopListening()}else if(!this.autodetect){// press & hold mode, stop listening when mouse is up
this.stopListening()}this.buttonEl.blur();this.pressed=!1}},{key:"startRecordingVoice",value:function startRecordingVoice(){this.isRecording=!0;this.voiceRecorder.startRecord()}// need audio blob in Base64 to send to CloudSpeech API
},{key:"blobToBase64",value:function blobToBase64(blob,cb){// return cb(blob)
var reader=new FileReader;reader.onloadend=function(){var dataUrl=reader.result,base64=dataUrl.split(",")[1];cb(base64)};reader.onerror=function(err){console.error("Error in reading blob",err)};reader.readAsDataURL(blob)}},{key:"onStopRecordingVoiceComplete",value:function onStopRecordingVoiceComplete(blob){if(this.isRecording){this.isRecording=!1;this._setState(ListeningState.IDLE);this.circleViz.stopAnimation();this.postTranscription(blob)}}},{key:"postTranscription",value:function(){var _postTranscription=babelHelpers.asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee(blob){var _this37=this,buffer,encodeMono,wav,samples,mp3Blob;return regeneratorRuntime.wrap(function _callee$(_context){while(1){switch(_context.prev=_context.next){case 0:encodeMono=function _ref(channels,sampleRate,samples){for(var buffer=[],mp3enc=new lamejs.Mp3Encoder(channels,sampleRate,128),remaining=samples.length,maxSamples=1152,i=0;remaining>=maxSamples;i+=maxSamples){var mono=samples.subarray(i,i+maxSamples),mp3buf=mp3enc.encodeBuffer(mono);if(0<mp3buf.length){buffer.push(new Int8Array(mp3buf))}remaining-=maxSamples}var d=mp3enc.flush();if(0<d.length){buffer.push(new Int8Array(d))}console.log("done encoding, size=",buffer.length);return new Blob(buffer,{type:"audio/mp3"})};_context.next=3;return blob.arrayBuffer();case 3:buffer=_context.sent;console.log("got arrayBuffer");wav=lamejs.WavHeader.readHeader(new DataView(buffer));console.log("wav:",wav);samples=new Int16Array(buffer,wav.dataOffset,wav.dataLen/2);mp3Blob=encodeMono(wav.channels,wav.sampleRate,samples);this.showLoader();this.blobToBase64(mp3Blob,function(data){/* let postBody = `{
          "config": {
            "encoding": "LINEAR16",
            "languageCode": "en-US",
          ...
          "audio": {
            "content": "${data}"
               ...
               } */var postBody="".concat(data);try{var string=postBody;console.log("Size of sample is: "+string.length);var compressed=LZString.compress(string);console.log("Size of compressed plain sample is: "+compressed.length);compressed=LZString.compressToEncodedURIComponent(string);console.log("Size of compressed uri sample is: "+compressed.length);// string = LZString.decompress(compressed);
// console.log("Sample is: " + string);
// var output = pako.inflate(postBody, { to: "string" })
// console.log("zip", output.length, "&", postBody.length);
postBody=compressed}catch(error){console.log("zip error",error)}_this37.hideLoader();_this37.dispatchEvent(new CustomEvent("onSpeech",{detail:postBody}));/* 
           ...
           xhr.onload = data => {
            ...
            xhr.send(postBody); */});case 11:case"end":return _context.stop();}}},_callee,this)}));function postTranscription(_x){return _postTranscription.apply(this,arguments)}return postTranscription}()},{key:"startListening",value:function startListening(){if(this.clickForPermission){this.circleViz.attachUserMediaStream()}this._setState(ListeningState.LISTENING);if(!this.useCloudSpeech){// start recognition
this.recognition.start()}else{this.startRecordingVoice()}this.circleViz.startAnimation()}},{key:"stopListening",value:function stopListening(){if(!this.useCloudSpeech){this._setState(ListeningState.IDLE);this.circleViz.stopAnimation();this.recognition.stop();this.removeSpeechEventListeners()}else{this.voiceRecorder.stopRecord()}}},{key:"onSpeechResult",value:function onSpeechResult(event){var result={speechResult:event.results[0][0].transcript,confidence:event.results[0][0].confidence,isFinal:event.results[0].isFinal,sourceEvent:event};this._setState(ListeningState.USER_INPUT);this.dispatchEvent(new CustomEvent("onSpeech",{detail:result}));if(result.isFinal){this._setState(ListeningState.IDLE);this.circleViz.stopAnimation();if(this.autodetect){this.stopListening()}}}},{key:"onSpeechEnd",value:function onSpeechEnd(){this._setState(ListeningState.IDLE);this.circleViz.stopAnimation();this.removeSpeechEventListeners()}},{key:"onSpeechError",value:function onSpeechError(event){this._setState(ListeningState.IDLE);this.circleViz.stopAnimation();this.removeSpeechEventListeners();this.dispatchEvent(new CustomEvent("onSpeechError",{detail:event}));if("not-allowed"===event.error){this.$["mic-not-allowed"].classList.add("show");this._setState(ListeningState.DISABLED);this.set("disabled",!0)}}},{key:"removeSpeechEventListeners",value:function removeSpeechEventListeners(){if(this.recognition){this.recognition.removeEventListener("result",this.onSpeechResult);this.recognition.removeEventListener("end",this.onSpeechEnd);this.recognition.removeEventListener("error",this.onSpeechError)}}},{key:"_onEnabledStateChange",value:function _onEnabledStateChange(newValue,oldValue){if(this.disabled)this._setState(ListeningState.DISABLED);else this._setState(ListeningState.IDLE)}},{key:"_onStateChange",value:function _onStateChange(newValue,oldValue){this.dispatchEvent(new CustomEvent("onStateChange",{detail:{newValue:newValue,oldValue:oldValue}}))}},{key:"showLoader",value:function showLoader(){this.googleMicLoaderEl.style.opacity=1;this.googleMicEl.style.display="none"}},{key:"hideLoader",value:function hideLoader(){this.googleMicLoaderEl.style.opacity=0;this.googleMicEl.style.display="block"}},{key:"checkWebRTC",value:function checkWebRTC(){var audioCtx=window.AudioContext||window.webkitAudioContext;if(audioCtx&&(navigator.mediaDevices&&navigator.mediaDevices.getUserMedia||navigator.getUserMedia)&&Worker){return!0}console.error("Need to provide a cloud-speech-api-key for support on this browser.");return!1}}]);return VoiceButton}(PolymerElement);window.mobileAndTabletcheck=function(){var check=!1;(function(a){if(/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino|android|ipad|playbook|silk/i.test(a)||/1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(a.substr(0,4)))check=!0})(navigator.userAgent||navigator.vendor||window.opera);return check};customElements.define(VoiceButton.is,VoiceButton)});