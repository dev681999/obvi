...
n=n.parentNode||n.host}return n===this.node}/**
   ...
   */DomApi.prototype.cloneNode;/**
    ...
    *
    */_exports.dom=dom;var polymer_dom={matchesSelector:matchesSelector,DomApi:DomApi,dom:dom,flush:flush$1,addDebouncer:enqueueDebouncer};_exports.$polymerDom=polymer_dom;const bundledImportMeta$1=_objectSpread({},meta,{url:new URL("./node_modules/%40polymer/polymer/lib/legacy/legacy-element-mixin.js",meta.url).href});let styleInterface=window.ShadyCSS;/**
...
}}};/** @polymerBehavior */_exports.IronButtonStateImpl=IronButtonStateImpl;const IronButtonState=[IronA11yKeysBehavior,IronButtonStateImpl];_exports.IronButtonState=IronButtonState;var ironButtonState={IronButtonStateImpl:IronButtonStateImpl,IronButtonState:IronButtonState};_exports.$ironButtonState=ironButtonState;var Utility={distance:function(x1,y1,x2,y2){var xDelta=x1-x2,yDelta=y1-y2;return Math.sqrt(xDelta*xDelta+yDelta*yDelta)},now:window.performance&&window.performance.now?window.performance.now.bind(window.performance):Date.now};/**
    * @param {HTMLElement} element
    * @constructor
...
var f=String.fromCharCode,keyStrBase64="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=",keyStrUriSafe="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+-$",baseReverseDic={};function getBaseValue(alphabet,character){if(!baseReverseDic[alphabet]){baseReverseDic[alphabet]={};for(var i=0;i<alphabet.length;i++){baseReverseDic[alphabet][alphabet.charAt(i)]=i}}return baseReverseDic[alphabet][character]}var LZString={compressToBase64:function(input){if(null==input)return"";var res=LZString._compress(input,6,function(a){return keyStrBase64.charAt(a)});switch(res.length%4){// To produce valid Base64
default:// When could this happen ?
case 0:return res;case 1:return res+"===";case 2:return res+"==";case 3:return res+"=";}},decompressFromBase64:function(input){if(null==input)return"";if(""==input)return null;return LZString._decompress(input.length,32,function(index){return getBaseValue(keyStrBase64,input.charAt(index))})},compressToUTF16:function(input){if(null==input)return"";return LZString._compress(input,15,function(a){return f(a+32)})+" "},decompressFromUTF16:function(compressed){if(null==compressed)return"";if(""==compressed)return null;return LZString._decompress(compressed.length,16384,function(index){return compressed.charCodeAt(index)-32})},//compress into uint8array (UCS-2 big endian format)
compressToUint8Array:function(uncompressed){// 2 bytes per character
...
decompressFromUint8Array:function(compressed){if(null===compressed||compressed===void 0){return LZString.decompress(compressed)}else{// 2 bytes per character
...
// import './lame.js';
// import './lz-string.js';
const $_documentContainer=document.createElement("template");$_documentContainer.setAttribute("style","display: none;");$_documentContainer.innerHTML=`<dom-module id="voice-button">
...
var MIN_SPEAKING_VOLUME=.01,sum=0,i,clipcount=0;for(i=0;i<left.length;++i){sum+=left[i]*left[i];if(.99<Math.abs(left[i])){clipcount+=1}}var volume=Math.sqrt(sum/left.length);if(volume>MIN_SPEAKING_VOLUME){this.isSpeaking=!0;clearTimeout(this.speakingTimeout)}else{if(this.isSpeaking){this.isSpeaking=!1;clearTimeout(this.speakingTimeout);console.log("adding timeout of ",this.timeoutval,this.assessmentToken);this.speakingTimeout=setTimeout(()=>{this.stopRecord()},this.timeoutval)}}};this.logError=error=>{console.error(error)};this.startRecord=()=>{this.audioContext=new AudioContext;/** 
                                             ...
                                             * */if(this.audioContext.createJavaScriptNode){this.processor=this.audioContext.createJavaScriptNode(this.config.bufferLen,this.config.numChannels,this.config.numChannels)}else if(this.audioContext.createScriptProcessor){this.processor=this.audioContext.createScriptProcessor(this.config.bufferLen,this.config.numChannels,this.config.numChannels)}else{console.log("WebAudio API has no support on this browser.")}this.processor.connect(this.audioContext.destination);/**
                                                            ...
                                                            * */navigator.mediaDevices.getUserMedia({audio:!0,video:!1}).then(this.gotStreamMethod.bind(this)).catch(this.logError)};this.getBuffers=event=>{for(var buffers=[],ch=0;2>ch;++ch){buffers[ch]=event.inputBuffer.getChannelData(ch)}return buffers};this.gotStreamMethod=stream=>{// audioElement.src = "";
this.isRecording=!0;this.tracks=stream.getTracks();/** 
...
// Opera 8.0+
var isOpera=!!window.opr&&!!opr.addons||!!window.opera||0<=navigator.userAgent.indexOf(" OPR/");const SpeechRecognition=window.webkitSpeechRecognition;// if (typeof SpeechRecognition === 'undefined' || isOpera) {
...
const isMobile=window.mobileAndTabletcheck();this.recognition.interimResults=!isMobile;this.recognition.continuous=isMobile?!1:!this.autodetect;this.recognition.maxAlternatives=0;// add event listeners
...
this.stopListening()}this.buttonEl.blur();this.pressed=!1}startRecordingVoice(){this.isRecording=!0;this.voiceRecorder.startRecord()}// need audio blob in Base64 to send to CloudSpeech API
blobToBase64(blob,cb){// return cb(blob)
let reader=new FileReader;reader.onloadend=()=>{let dataUrl=reader.result,base64=dataUrl.split(",")[1];cb(base64)};reader.onerror=err=>{console.error("Error in reading blob",err)};reader.readAsDataURL(blob)}onStopRecordingVoiceComplete(blob){if(this.isRecording){this.isRecording=!1;this._setState(ListeningState.IDLE);this.circleViz.stopAnimation();this.postTranscription(blob)}}postTranscription(blob){var _this=this;return babelHelpers.asyncToGenerator(function*(){var buffer=yield blob.arrayBuffer();console.log("got arrayBuffer");function encodeMono(channels,sampleRate,samples){for(var buffer=[],mp3enc=new lamejs.Mp3Encoder(channels,sampleRate,128),remaining=samples.length,maxSamples=1152,i=0;remaining>=maxSamples;i+=maxSamples){var mono=samples.subarray(i,i+maxSamples),mp3buf=mp3enc.encodeBuffer(mono);if(0<mp3buf.length){buffer.push(new Int8Array(mp3buf))}remaining-=maxSamples}var d=mp3enc.flush();if(0<d.length){buffer.push(new Int8Array(d))}console.log("done encoding, size=",buffer.length);return new Blob(buffer,{type:"audio/mp3"})}var wav=lamejs.WavHeader.readHeader(new DataView(buffer));console.log("wav:",wav);var samples=new Int16Array(buffer,wav.dataOffset,wav.dataLen/2),mp3Blob=encodeMono(wav.channels,wav.sampleRate,samples);_this.showLoader();_this.blobToBase64(mp3Blob,data=>{/* let postBody = `{
          "config": {
            "encoding": "LINEAR16",
            "languageCode": "en-US",
          ...
          "audio": {
            "content": "${data}"
               ...
               } */let postBody=`${data}`;try{var string=postBody;console.log("Size of sample is: "+string.length);var compressed=LZString.compress(string);console.log("Size of compressed plain sample is: "+compressed.length);compressed=LZString.compressToEncodedURIComponent(string);console.log("Size of compressed uri sample is: "+compressed.length);// string = LZString.decompress(compressed);
// console.log("Sample is: " + string);
// var output = pako.inflate(postBody, { to: "string" })
// console.log("zip", output.length, "&", postBody.length);
postBody=compressed}catch(error){console.log("zip error",error)}_this.hideLoader();_this.dispatchEvent(new CustomEvent("onSpeech",{detail:postBody}));/* 
           ...
           xhr.onload = data => {
            ...
            xhr.send(postBody); */})})()}startListening(){if(this.clickForPermission){this.circleViz.attachUserMediaStream()}this._setState(ListeningState.LISTENING);if(!this.useCloudSpeech){// start recognition
this.recognition.start()}else{this.startRecordingVoice()}this.circleViz.startAnimation()}stopListening(){if(!this.useCloudSpeech){this._setState(ListeningState.IDLE);this.circleViz.stopAnimation();this.recognition.stop();this.removeSpeechEventListeners()}else{this.voiceRecorder.stopRecord()}}onSpeechResult(event){var result={speechResult:event.results[0][0].transcript,confidence:event.results[0][0].confidence,isFinal:event.results[0].isFinal,sourceEvent:event};this._setState(ListeningState.USER_INPUT);this.dispatchEvent(new CustomEvent("onSpeech",{detail:result}));if(result.isFinal){this._setState(ListeningState.IDLE);this.circleViz.stopAnimation();if(this.autodetect){this.stopListening()}}}onSpeechEnd(){this._setState(ListeningState.IDLE);this.circleViz.stopAnimation();this.removeSpeechEventListeners()}onSpeechError(event){this._setState(ListeningState.IDLE);this.circleViz.stopAnimation();this.removeSpeechEventListeners();this.dispatchEvent(new CustomEvent("onSpeechError",{detail:event}));if("not-allowed"===event.error){this.$["mic-not-allowed"].classList.add("show");this._setState(ListeningState.DISABLED);this.set("disabled",!0)}}removeSpeechEventListeners(){if(this.recognition){this.recognition.removeEventListener("result",this.onSpeechResult);this.recognition.removeEventListener("end",this.onSpeechEnd);this.recognition.removeEventListener("error",this.onSpeechError)}}_onEnabledStateChange(newValue,oldValue){if(this.disabled)this._setState(ListeningState.DISABLED);else this._setState(ListeningState.IDLE)}_onStateChange(newValue,oldValue){this.dispatchEvent(new CustomEvent("onStateChange",{detail:{newValue:newValue,oldValue:oldValue}}))}showLoader(){this.googleMicLoaderEl.style.opacity=1;this.googleMicEl.style.display="none"}hideLoader(){this.googleMicLoaderEl.style.opacity=0;this.googleMicEl.style.display="block"}checkWebRTC(){let audioCtx=window.AudioContext||window.webkitAudioContext;if(audioCtx&&(navigator.mediaDevices&&navigator.mediaDevices.getUserMedia||navigator.getUserMedia)&&Worker){return!0}console.error("Need to provide a cloud-speech-api-key for support on this browser.");return!1}}window.mobileAndTabletcheck=function(){var check=!1;(function(a){if(/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino|android|ipad|playbook|silk/i.test(a)||/1207|6310|6590|3gso|4thp|50[1-6]i|770s|802s|a wa|abac|ac(er|oo|s\-)|ai(ko|rn)|al(av|ca|co)|amoi|an(ex|ny|yw)|aptu|ar(ch|go)|as(te|us)|attw|au(di|\-m|r |s )|avan|be(ck|ll|nq)|bi(lb|rd)|bl(ac|az)|br(e|v)w|bumb|bw\-(n|u)|c55\/|capi|ccwa|cdm\-|cell|chtm|cldc|cmd\-|co(mp|nd)|craw|da(it|ll|ng)|dbte|dc\-s|devi|dica|dmob|do(c|p)o|ds(12|\-d)|el(49|ai)|em(l2|ul)|er(ic|k0)|esl8|ez([4-7]0|os|wa|ze)|fetc|fly(\-|_)|g1 u|g560|gene|gf\-5|g\-mo|go(\.w|od)|gr(ad|un)|haie|hcit|hd\-(m|p|t)|hei\-|hi(pt|ta)|hp( i|ip)|hs\-c|ht(c(\-| |_|a|g|p|s|t)|tp)|hu(aw|tc)|i\-(20|go|ma)|i230|iac( |\-|\/)|ibro|idea|ig01|ikom|im1k|inno|ipaq|iris|ja(t|v)a|jbro|jemu|jigs|kddi|keji|kgt( |\/)|klon|kpt |kwc\-|kyo(c|k)|le(no|xi)|lg( g|\/(k|l|u)|50|54|\-[a-w])|libw|lynx|m1\-w|m3ga|m50\/|ma(te|ui|xo)|mc(01|21|ca)|m\-cr|me(rc|ri)|mi(o8|oa|ts)|mmef|mo(01|02|bi|de|do|t(\-| |o|v)|zz)|mt(50|p1|v )|mwbp|mywa|n10[0-2]|n20[2-3]|n30(0|2)|n50(0|2|5)|n7(0(0|1)|10)|ne((c|m)\-|on|tf|wf|wg|wt)|nok(6|i)|nzph|o2im|op(ti|wv)|oran|owg1|p800|pan(a|d|t)|pdxg|pg(13|\-([1-8]|c))|phil|pire|pl(ay|uc)|pn\-2|po(ck|rt|se)|prox|psio|pt\-g|qa\-a|qc(07|12|21|32|60|\-[2-7]|i\-)|qtek|r380|r600|raks|rim9|ro(ve|zo)|s55\/|sa(ge|ma|mm|ms|ny|va)|sc(01|h\-|oo|p\-)|sdk\/|se(c(\-|0|1)|47|mc|nd|ri)|sgh\-|shar|sie(\-|m)|sk\-0|sl(45|id)|sm(al|ar|b3|it|t5)|so(ft|ny)|sp(01|h\-|v\-|v )|sy(01|mb)|t2(18|50)|t6(00|10|18)|ta(gt|lk)|tcl\-|tdg\-|tel(i|m)|tim\-|t\-mo|to(pl|sh)|ts(70|m\-|m3|m5)|tx\-9|up(\.b|g1|si)|utst|v400|v750|veri|vi(rg|te)|vk(40|5[0-3]|\-v)|vm40|voda|vulc|vx(52|53|60|61|70|80|81|83|85|98)|w3c(\-| )|webc|whit|wi(g |nc|nw)|wmlb|wonu|x700|yas\-|your|zeto|zte\-/i.test(a.substr(0,4)))check=!0})(navigator.userAgent||navigator.vendor||window.opera);return check};customElements.define(VoiceButton.is,VoiceButton)});